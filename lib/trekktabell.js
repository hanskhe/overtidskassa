 ...
 * - Net take-home overtime pay (using actual tax, not withholding)
 * - Effective marginal tax rate
 *
 * IMPORTANT: The withholding is typically ~14.3% higher than actual tax because
 * employers assume overtime recurs monthly. Users get this difference back as a
 * refund at tax settlement. By default, we show the actual tax (more accurate).
 *
...
function calculateOvertimeTakeHome({ yearlySalary, overtimeHours, tableNumber, taxYear = 2026 }) {
  ...
  // This is the true tax you owe on the overtime income.
  // You'll get the difference (withholding - actualTax) back as a refund.
  ...
  return {
    ...
    // Actual tax (what you truly owe - DEFAULT)
    actualTax: Math.round(actualTaxOnOvertime * 100) / 100,
    takeHome: Math.round(takeHomeActual * 100) / 100,  // Default to actual
    effectiveRate: Math.round(effectiveRateActual * 1000) / 1000,  // Default to actual

    // Withholding (what employer deducts from paycheck)
    ...
    // Refund estimate (difference between withholding and actual tax)
    estimatedRefund: Math.round((withholdingOnOvertime - actualTaxOnOvertime) * 100) / 100
  };
}
