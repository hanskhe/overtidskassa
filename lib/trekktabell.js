...
// Import tax rates (Node.js only - in browser, tax-rates.js is loaded first via manifest)
if (typeof TAX_RATES === 'undefined') {
  if (typeof require !== 'undefined') {
    // Node.js environment
    const taxRatesModule = require('./tax-rates.js');
    var TAX_RATES = taxRatesModule.TAX_RATES;
    var getTaxRates = taxRatesModule.getTaxRates;
  } else {
    // Browser without tax-rates.js loaded - this shouldn't happen
    throw new Error('TAX_RATES not found. Make sure tax-rates.js is loaded before trekktabell.js');
  }
}
...
function calculateMonthlyWithholding(monthlyGross, tableNumber, taxYear = 2026) {
  ...
  const rates = getTaxRates ? getTaxRates(taxYear) : TAX_RATES[taxYear];
  if (!rates) {
...
function validateParameters({ yearlySalary, overtimeHours, tableNumber, taxYear }) {
  ...
  if (!yearlySalary || yearlySalary < 0) {
    errors.push('Yearly salary must be a positive number');
  }

  if (yearlySalary < 100000 || yearlySalary > 5000000) {
    errors.push('Yearly salary should be between 100,000 and 5,000,000 NOK');
  }
