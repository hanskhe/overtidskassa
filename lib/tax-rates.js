...
/**
 * Get the current tax year (defaults to latest available)
 * @returns {number} Current tax year
 */
function getCurrentTaxYear() {
  const years = Object.keys(TAX_RATES).map(Number);
  return Math.max(...years);
}

// Export for use in other modules
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { TAX_RATES, getTaxRates, getCurrentTaxYear };
}
